...
public class ClientSocketConnection {
	...
	public void stopClientSocket() {
		...
		//cc.txtSend.setDisable(true);
		//cc.btnSend.setDisable(true);
		//cc.btnSendFile.setDisable(true);
	...
	public String createXMLMessage(String message) {
		...
		for (Member eachMember : members.values()) {
			if (eachMember.getName().equals(cc.txtName.getText()))
				continue;
	...
	public void setChatController(ChatController cc, String peeruser)
	{
		this.cc = cc;
	}
	...
	public void performSendFile(String fileNameSender) {
		try {
			...
			cc.addStringConsole("Sending the " + fileName + " completed", ServerConstants.SYSTEM_MESSAGE);
	...
	private class SocketThread extends Thread {
		...
		public void run() {
			try {
				...
				//cc.addStringConsole("Chat Room starts on port: " + port, ServerConstants.SYSTEM_MESSAGE);
							...
							try {
								...
								if (message == ServerConstants.NEW_MEMBER) {
									...
									Platform.runLater(new Runnable() {
										public void run() {
											try {
												ClientController.setloginuser(textMessage);
									...
									//cc.addStringConsole("The server stops working", ServerConstants.ERROR_MESSAGE);
									Platform.runLater(new Runnable() {
										public void run() {
									...
									if (flag)
									{
										cc.addStringConsole(textMessage, ServerConstants.BROADCAST_MESSAGE);
									}
								} else if (message == ServerConstants.SYSTEM_MESSAGE) {

									//cc.addStringConsole(textMessage, ServerConstants.SYSTEM_MESSAGE);

								} else if (message == ServerConstants.PRIVATE_MESSAGE) {
									List<String> listStr = new ArrayList<String>(Arrays.asList(textMessage.split(">")));
									String sender = listStr.get(0);
									String loginuser = listStr.get(1);
									boolean flag = true;
									for (Member member : members.values()) {
										if (member.getName().equals(sender) && member.getIsBlocked()) {
											flag = false;
										...
										if(cc == null)
										{
											Platform.runLater(new Runnable() {
												public void run() {
													...
													Parent ClientChat;
													try {
														ClientChat = (Parent)fxmlLoaderClientChat.load();

														ChatController cc = fxmlLoaderClientChat.<ChatController>getController();
														ClientSocketConnection.this.setChatController(cc, sender);
														//notice this csc
														cc.setClientSocketConnection(ClientSocketConnection.this);
													...
													cc.setpeeruser(sender, loginuser);
													cc.addStringConsole(textMessage, ServerConstants.PRIVATE_MESSAGE);
												}
											});
										}
										else
									...
									cc.addStringConsole(consoleMessage, ServerConstants.SEND_ACK_FILE_TO_CLIENT);
									cc.addLinkLoadFile(fileName);
									...
									cc.addStringConsole("Start downloading the "+fileName, ServerConstants.SYSTEM_MESSAGE);
										
									///////////////////
									String workingDirectory = System.getProperty("user.dir");
									
									File directory = new File(workingDirectory + File.separator + cc.txtName.getText());
									if(!directory.isDirectory()) {
										directory.mkdir();
									}
									System.out.println(directory + File.separator + fileName);
							        ...
							        cc.addStringConsole("Download "+fileName+" completed", ServerConstants.SYSTEM_MESSAGE);
							        cc.addLinkOpenFile(fileName);

								} else {

									cc.addStringConsole(textMessage + " with code " + message,ServerConstants.SYSTEM_MESSAGE);
				...
				cc.addStringConsole("Cannot connect to the server: Code 3", ServerConstants.ERROR_MESSAGE);
				cc.btnSend.setDisable(true);
				cc.txtSend.setDisable(true);
				cc.btnSendFile.setDisable(true);
			}
